...
- ドキュメントやコメントアウトは、過剰な表現を使わず端的に記述する。
- 静的な解析を用いて、1ファイルには1関数のみを定義することを義務付ける。
- docsやmd、jsonを除いたバックエンドに関して、1ファイル100行まで。
- 循環的複雑度は5を上回ってはならない。
- DRY原則、SOLID原則を厳守する。
- 純粋関数を用いることを義務付ける。（テストを容易にするため）
- 後方互換やフォールバックは異常系を握り潰してしまうため不要。
- 必ず各実装のたびに想定される全ての入力に対してテストを行う。
- 変更作業の前に必ず新規ブランチを作成する
- 直接 main へ push しない
- PR を作成して提出する
- ブランチ名と PR タイトルは gitmoji 準拠
- PR 本文も gitmoji を冒頭に付ける（例: :memo: など）
- どんなに小さな変更や機能追加であっても、必ずコードを編集する前にブランチを切ってPRを出す。
- これらの指示が守られているかどうかを静的解析で判定する。
